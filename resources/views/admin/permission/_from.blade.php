    ...
    <div class="layui-input-inline">
        <select name="parent_id">
            ...
            @forelse($permissions as $p1)
                <option value="{{$p1->id}}" {{ isset($permission->id) && $p1->id == $permission->parent_id ? 'selected' : '' }}  {{ isset($parent_id) && $p1->id == $parent_id ? 'selected' : '' }} >{{$p1->display_name}}</option>
                @if($p1->childs->isNotEmpty())
                    @foreach($p1->childs as $p2)
                        <option value="{{$p2->id}}" {{ isset($permission->id) && $p2->id == $permission->parent_id ? 'selected' : '' }} {{ isset($parent_id) && $p2->id == $parent_id ? 'selected' : '' }} >&nbsp;&nbsp;&nbsp;┗━━{{$p2->display_name}}</option>
                        @if($p2->childs->isNotEmpty())
                            @foreach($p2->childs as $p3)
                                <option value="{{$p3->id}}" {{ isset($permission->id) && $p3->id == $permission->parent_id ? 'selected' : '' }} {{ isset($parent_id) && $p3->id == $parent_id ? 'selected' : '' }}>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;┗━━{{$p3->display_name}}</option>
                            @endforeach
                        @endif
                    @endforeach
                @endif
...
<div class="layui-form-item">
    ...
    <div class="layui-input-inline">
        <select name="module" >
            <option value="1" {{ isset($permission->module) &&  $permission->module ==1 ? 'selected' : '' }} >系统</option>
            <option value="2" {{ isset($permission->module) &&  $permission->module ==2 ? 'selected' : '' }}  >商城</option>
            <option value="3" {{ isset($permission->module) &&  $permission->module ==3 ? 'selected' : '' }}  >物联网</option>
...
<div class="layui-form-item">
    ...
    <div class="layui-input-inline">
        <input type="radio" name="status" value="1" title="显示" @if(!isset($permission) || (isset($permission) && $permission->status==1)) checked @endif >
        <input type="radio" name="status" value="0" title="隐藏" @if(isset($permission) && $permission->status==0) checked @endif>
    </div>
</div>
<div class="layui-form-item">
