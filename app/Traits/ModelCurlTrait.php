...
trait ModelCurlTrait
{
    ...
    public function getModelField(){
        ...
        $data = Schema::getColumnListing($this->getModel()->getTable());
        return $data;
    }
    ...
    protected function saveData($request, $model, $id = '')
    {
        ...
        //附加检验
        $extend_check = $this->extendValidate($request,$model, $id);
        
        if ($extend_check !== true) {
            
           // return $this->returnApi(1,$extend_check);
           if ($request->ajax() || $request->wantsJson()) {
                return $this->returnApi(1,$extend_check);
            }else{
               
                return Redirect::back()->withErrors($extend_check);
            }
        }
        //设置请求参数
    ...
    protected function saveMessage($result, $id = '')
    {
        ...
        if ($request->ajax() || $request->wantsJson()) {
            ...
            if($result){
                return Redirect::to(URL::route(substr(\Request::route()->getName(),0,strripos(\Request::route()->getName(), '.'))))->with(['success'=>$msg_str]);
            }else{

             return Redirect::back()->withErrors($msg_str);
            }
        }
    }
